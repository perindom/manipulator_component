...
void *publish_task(void *arg0){
    ...
    ret |= MQTT_IF_Subscribe(mqttClientHandle, "Topic_VD", MQTT_QOS_0, topic_subscribe_callback);
    ret |= MQTT_IF_Subscribe(mqttClientHandle, "Sensor", MQTT_QOS_0, sensor_callback);
    ret |= MQTT_IF_Subscribe(mqttClientHandle, "Average", MQTT_QOS_0, sensor_callback);
